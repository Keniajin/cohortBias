...
r <- ggplot(total_bias_R20_10, aes(x = Se, y = Sp, z = q50)) +
    ...
    annotate("text", x = 1, y = 1, label = "0.1", size = 2.5) +
...
rr <- ggplot(total_bias_RR20_10, aes(x = Se, y = Sp, z = q50)) +
    ...
    annotate("text", x = 1, y = 1, label = "3", size = 2.5) +
    scale_x_continuous(breaks = seq(0.65, 1, .05), expand = c(0,0)) +
    scale_y_continuous(breaks = seq(0.75, 1, .05), expand = c(0,0)) +
    coord_fixed(ylim = c(0.75, 1.025), xlim = c(0.65, 1.025)) +
    scale_colour_gradient(guide = 'none') +
...
r <- ggplot(total_bias_R20_01, aes(x = Se, y = Sp, z = q50)) +
    ...
    annotate("text", x = 1, y = 1, label = "0.01", size = 2.5) +
    scale_x_continuous(breaks = seq(0.65, 1, .05), expand = c(0,0)) +
    scale_y_continuous(breaks = seq(0.75, 1, .05), expand = c(0,0)) +
    coord_fixed(ylim = c(0.75, 1.025), xlim = c(0.65, 1.025)) +
    scale_colour_gradient(guide = 'none') +
...
r <- ggplot(total_bias_R20_05, aes(x = Se, y = Sp, z = q50)) +
    ...
    annotate("text", x = 1, y = 1, label = "0.05", size = 2.5) +
    scale_x_continuous(breaks = seq(0.65, 1, .05), expand = c(0,0)) +
    scale_y_continuous(breaks = seq(0.75, 1, .05), expand = c(0,0)) +
    coord_fixed(ylim = c(0.75, 1.025), xlim = c(0.65, 1.025)) +
    scale_colour_gradient(guide = 'none') +
...
r <- ggplot(selection_bias_R05_10, aes(x = Se, y = Sp, z = q50)) +
    geom_contour(aes(colour = ..level..), breaks = c(.1, .105, .11, .116)) +
    scale_x_continuous(breaks = seq(0.65, 1, .05), expand = c(0,0)) +
    scale_y_continuous(breaks = seq(0.75, 1, .05), expand = c(0,0)) +
    coord_fixed(ylim = c(0.75, 1.025), xlim = c(0.65, 1.025)) +
    scale_colour_gradient(guide = 'none') +
...
A cohort study is the standard method to estimate the incidence of diseases and
identify their natural history, by analyzing the association between a baseline
exposure and risk of disease over the follow-up period.
A disease-free population is identified, i.e.\ subjects with the outcome at
baseline are excluded from the follow-up, while new, incident cases of exposure
are identified.
Therefore it is assumed that prevalent and non-prevalent cases can be
differentiated with no error so that only susceptible individuals are included in the
follow-up.
Incident cases are likewise supposed to be correctly identified.

However, using imperfect tests may lead to biased estimates of disease frequency
and measures of association.
Many studies have looked into the effect of misclassification on statistical
inferences.
These evaluations were either within a cross-sectional study framework,
assessing biased prevalence, or for cohort study designs, evaluating biased
incidence rate or risk ratio (RR) estimates based on misclassification at one of
...
of incidence and risk ratio.

\section{Material and Methods}

To investigate impact on measure of disease frequency, data sets from a
hypothetical cohort study with two samples collected one month apart were
simulated and analyzed based on specific test and disease characteristics, for a
stable population over the follow-up time, and with no elimination of disease or
...
Departure of the estimates of disease incidence and risk ratio from their true
value are mainly a function of test specificity, and disease prevalence and
incidence.
Importance of a good to excellent specificity over sensitivity.
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \end{center}
  \caption{Total bias for incidence rate according to sensitivity and
    specificity, for 5\% and 20\% disease prevalence.}
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \end{center}
  \caption{Total bias for risk ratio according to sensitivity and specificity,
    for 5\% and 20\% disease prevalence. True risk ratio = 3.}
...
\begin{figure}[htbp]
  ...
  \caption{Apparent incidence resulting from total bias, as a function of
    specificity. Disease prevalence = 5\%.}
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \end{center}
  \caption{Apparent incidence resulting from total bias, as a function of
    specificity. Disease prevalence = 20\%.}
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \end{center}
  \caption{Total bias for risk ratio as a function of specificity and disease
    risk, and for a sensitivity of 95\%. True risk ratio = 3.}
  \label{fig:apparent_incidence_20}
\end{figure}
